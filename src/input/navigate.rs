...
pub fn handle_normal_mode(app: &mut App, key: KeyEvent) {
    ...
    if key_match(&key, &app.config.keybindings.global.command_palette) {
    ...
    if key_match(&key, &app.config.keybindings.global.toggle_chrome) {
        app.ui_chrome_visible = !app.ui_chrome_visible;
        return;
    }
        ...
        && key_match(&key, &app.config.keybindings.tasks.filter_priority)
    {
        ...
        && key_match(&key, &app.config.keybindings.tasks.filter_overdue)
    {
        app.set_task_filter(models::TaskFilter::Overdue);
    } else if app.navigate_focus == models::NavigateFocus::Tasks
