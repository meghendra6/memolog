...
- **🔗 URL 하이라이팅**: 로그에 링크(`http://...`)가 있으면 파란색으로 예쁘게 표시해줍니다. 물론 클릭은 안됩니다. 터미널이잖아요. (대신 `Enter`로 폴더는 열어드립니다)

## 💭 사용 가이드
```markdown
- [ ] 할 일 이라고 적으면 우상단 todo list에서 확인하실 수 있습니다.
#어쩌구 와 같은 식으로 #태그 를 붙여 작성하시면 Navigate 모드의 t 버튼을 눌러 태그별로 로그를 모아볼 수 있습니다.
Navigate 모드에서 상하 화살표 혹은 마우스 스크롤을 통해 로그를 순회할 수 있는데, 완료처리하고자 하는 할 일 로그에서 엔터를 누르면 해당 할 일이 완료 처리됩니다.
```
...
- `/`: 검색 모드 진입
- `t`: 태그별로 모아보기
- `p`: 뽀모도로 (선택한 Task에 분 단위로 설정 후 시작/중지, 종료 시 🍅 추가)
- `g`: 활동 그래프 확인
- `o`: 로그 폴더 경로 확인 및 열기
- `Ctrl+q` 또는 `q`: 앱 종료
