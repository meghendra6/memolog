...
pub(crate) fn handle_editor_insert(app: &mut App, key: event::KeyEvent) {
    ...
    if key_match(&key, &app.config.keybindings.composer.indent) {
        let modified = if markdown::indent_or_outdent_list_line(&mut app.textarea, true) {
            true
        } else {
...
pub(crate) fn handle_editor_normal(app: &mut App, key: event::KeyEvent) {
...
pub(crate) fn exit_visual_mode(app: &mut App) {
    app.editor_mode = EditorMode::Normal;
    app.visual_anchor = None;
    app.pending_command = None;
    app.pending_count = 0;
...
fn open_line_above(app: &mut App) {
    ...
    let current_line = app
        ...
        .map(|s| s.as_str())
        .unwrap_or("");
    let prefix = markdown::list_continuation_prefix(current_line);
    app.textarea.move_cursor(CursorMove::Jump(row as u16, 0));
    app.textarea.insert_newline();
    app.textarea.move_cursor(CursorMove::Up);
    if !prefix.is_empty() {
